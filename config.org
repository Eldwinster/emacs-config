...
;;  ("oc" "Project changelog" entry #'+org-capture-central-project-changelog-file "* %U %?\n %i\n %a" :heading "Changelog" :prepend t))
  ...
  (defvar +org-capture-shopping
    (car (directory-files (expand-file-name org-directory) t ".*shopping.list.org")))
  (defvar +org-capture-to-read
    (car (directory-files (expand-file-name org-directory) t ".*to.read.org")))

  ;;; TODO change personal todo to personal and add children for todo, notes, idea, etc... all in your todo file add the according groups in org-super-agenda
           ...
           :children (
                      ...
                      ("blog" :keys "b"
                       :children (("default" :keys "d"
                                   ;; the heading need to be already created otherwise this won't work
                                   :olp ("Blog" "Default")
                                   :i-type "blog:%^{tag}")
                                  ("linux" :keys "l"
                                   :olp ("Blog" "Linux")
                                   :i-type "blog:linux:%^{tag}")
                                  ("ttrpg" :keys "t"
                                   :olp ("Blog" "TTRPG")
                                   :i-type "blog:TTRPG:%^{tag}")
                                  ("video game" :keys "v"
                                   :olp ("Blog" "Video Games")
                                   :i-type "blog:videoGame:%^{tag}")
                                  ("osr" :keys "o"
                                   :olp ("Blog" "OSR")
                                   :i-type "blog:osr:%^{tag}")
                                  ("security" :keys "s"
                                   :olp ("Blog" "Security")
                                   :i-type "blog:security:%^{tag}")))
                      ("cheatsheet" :keys "c"
                       :headline "Cheatsheet"
                       :i-type "cheatsheet:%^{tag}")
                      ("default" :keys "n"
                       :headline "Bookmarks"
                       :i-type "%^{tag}")
                      ...
                      ("repository" :keys "g"
                       :headline "Repository"
                       :children (("github" :keys "h"
                                   :i-type "github:repository:%^{tag}")
                                  ("gitlab" :keys "l"
                                   :i-type "gitlab:repository:%^{tag}")))
                      ("shop" :keys "p"
                       :headline "Shop"
                       :i-type "shop:%^{tag}")
                      ("video" :keys "v"
                       :headline "Video"
                       :i-type "video:youtube:%^{tag}")
                      ("video game" :keys "a"
...
This seems to cause problems for the rest of the ~org-roam-dailies~ functions.
#+begin_src emacs-lisp
